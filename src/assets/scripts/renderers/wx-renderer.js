let WxRenderer = function (opts) {
  ...
  let addFootnote = function (title, link) {
    ...
    for (let i = 0; i < footnotes.length; i++) {
      if (footnotes[i][2] === link) {
        // 如果找到一个重复的脚注，返回现有的脚注索引
        return footnotes[i][0];
      }
    }
    // 如果没有找到重复的脚注，添加新的脚注
    footnoteIndex += 1;
  ...
  this.getRenderer = function () {
    ...
    renderer.code = function (text, infoString) {
      ...
      for (let i = 0; i < lines.length; i++) {
        const line = lines[i];
        codeLines.push(
          `<code class="prettyprint"><span class="code-snippet_outer">${line || "<br>"
          }</span></code>`
        );
        numbers.push("<li></li>");
      }
    ...
    renderer.image = function (href, title, text) {
      let subText = "";
      if (title) {
        subText = `<figcaption ${getStyles("figcaption")}>${title}</figcaption>`;
      }
